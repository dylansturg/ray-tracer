...
void extract_path(char* source, char** dest);
...
/*		
	...
	while( fgets(current_line, OBJ_LINE_SIZE, obj_file_stream) )
	{
		...
		{
			char** path_str_ptr = (char**) malloc(1*sizeof(char*));
			extract_path(filename, path_str_ptr);
			strncpy(growable_data->material_filename, (*path_str_ptr), OBJ_FILENAME_LENGTH);
			strncat(growable_data->material_filename, strtok(NULL, WHITESPACE), OBJ_FILENAME_LENGTH);
...
void extract_path(char* source, char** dest){
	// dest = pointer to the string's memory address
	char path_separator;
#ifdef _WIN32
    path_separator = '\\';
#else
    path_separator = '/';
#endif
	char* end_of_path = strrchr(source, path_separator);
	if(end_of_path == NULL){
		(*dest) = (char*) malloc(2 * sizeof(char));
		strcpy(*dest, "");
		return; // no path, so just an empty string is sufficient
	}

	int path_len = end_of_path - source + 1;
	(*dest) = (char*) malloc((path_len+2) * sizeof(char));
	strncpy((*dest), source, path_len);
	(*dest)[path_len+1] = '\0'; // maybe hacky?
}
